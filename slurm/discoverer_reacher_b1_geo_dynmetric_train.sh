#!/bin/bash
# Discoverer/Slurm launcher for B1 plus dynamics-metric matching.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=lewm_b1dyn
...
#SBATCH --mem=128G
#SBATCH --gres=gpu:1
#SBATCH -o logs/lewm_b1dyn.%j.out
#SBATCH -e logs/lewm_b1dyn.%j.err
...
ABLATION_NAME="${ABLATION_NAME:-b1_geo_dynmetric_w1e3}" \
MODEL_CONFIG="${MODEL_CONFIG:-lewm_b1_effect}" \
OUTPUT_MODEL_NAME="${OUTPUT_MODEL_NAME:-lewm_reacher_b1_geo_dynmetric_w1e3}" \
RUN_ID="${RUN_ID:-reacher_workshop/b1_geo_dynmetric_w1e3_seed3072}" \
...
GEO_RESOLUTION="${GEO_RESOLUTION:-dynmetric}" \
GEO_WEIGHT="${GEO_WEIGHT:-1e-3}" \
GEO_K="${GEO_K:-4}" \
