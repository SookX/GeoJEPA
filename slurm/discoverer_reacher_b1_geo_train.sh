#!/bin/bash
# Discoverer/Slurm launcher for B1 plus Resolution-B geometry losses.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=lewm_b1geo
...
#SBATCH --mem=128G
#SBATCH --gres=gpu:1
#SBATCH -o logs/lewm_b1geo.%j.out
#SBATCH -e logs/lewm_b1geo.%j.err
...
ABLATION_NAME="${ABLATION_NAME:-b1_geo_full}" \
MODEL_CONFIG="${MODEL_CONFIG:-lewm_b1_effect}" \
OUTPUT_MODEL_NAME="${OUTPUT_MODEL_NAME:-lewm_reacher_${ABLATION_NAME:-b1_geo_full}}" \
RUN_ID="${RUN_ID:-reacher_workshop/${ABLATION_NAME:-b1_geo_full}_seed3072}" \
...
GEO_ANISO_WEIGHT="${GEO_ANISO_WEIGHT:-1e-4}" \
GEO_SCALE_WEIGHT="${GEO_SCALE_WEIGHT:-1e-3}" \
