#!/bin/bash
# Discoverer/Slurm launcher for B1: effect representation, prediction-only.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=lewm_b1
...
#SBATCH --mem=96G
#SBATCH --gres=gpu:1
#SBATCH -o logs/lewm_b1.%j.out
#SBATCH -e logs/lewm_b1.%j.err
...
ABLATION_NAME="${ABLATION_NAME:-b1_effect}" \
MODEL_CONFIG="${MODEL_CONFIG:-lewm_b1_effect}" \
OUTPUT_MODEL_NAME="${OUTPUT_MODEL_NAME:-lewm_reacher_b1_effect}" \
RUN_ID="${RUN_ID:-reacher_workshop/b1_effect_seed3072}" \
