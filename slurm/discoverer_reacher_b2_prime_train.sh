#!/bin/bash
# Discoverer/Slurm launcher for B2': cleaned/shifted value shaping + value-aware planning.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=lewm_b2p
...
#SBATCH --mem=96G
#SBATCH --gres=gpu:1
#SBATCH -o logs/lewm_b2p.%j.out
#SBATCH -e logs/lewm_b2p.%j.err
...
ABLATION_NAME="${ABLATION_NAME:-b2_prime_effect_value}" \
MODEL_CONFIG="${MODEL_CONFIG:-lewm_b2_prime_effect_value}" \
OUTPUT_MODEL_NAME="${OUTPUT_MODEL_NAME:-lewm_reacher_b2_prime_effect_value}" \
RUN_ID="${RUN_ID:-reacher_workshop/b2_prime_effect_value_seed3072}" \
