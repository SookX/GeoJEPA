#!/bin/bash
# Discoverer/Slurm launcher for B3: goal-conditioned value planning over B1 effects.

#SBATCH --partition=common
#SBATCH --qos=bg-eng-01
#SBATCH --account=bg-eng-01
#SBATCH --job-name=lewm_b3gv
...
#SBATCH --mem=128G
#SBATCH --gres=gpu:1
#SBATCH -o logs/lewm_b3gv.%j.out
#SBATCH -e logs/lewm_b3gv.%j.err
...
ABLATION_NAME="${ABLATION_NAME:-b3_goal_value}" \
MODEL_CONFIG="${MODEL_CONFIG:-lewm_b3_goal_value}" \
OUTPUT_MODEL_NAME="${OUTPUT_MODEL_NAME:-lewm_reacher_b3_goal_value}" \
RUN_ID="${RUN_ID:-reacher_workshop/b3_goal_value_seed3072}" \
...
VALUE_HORIZON="${VALUE_HORIZON:-10}" \
PLANNING_VALUE_WEIGHT="${PLANNING_VALUE_WEIGHT:-0.1}" \
PLANNING_VALUE_GAMMA="${PLANNING_VALUE_GAMMA:-0.99}" \
DATASET_NUM_STEPS="${DATASET_NUM_STEPS:-14}" \
